...
url = "https://www.cbssports.com/nfl/schedule/2025/regular/"  #base URL
#url = "https://www.cbssports.com/nfl/schedule/2024/preseason/"  #base URL
...
games = read_html(glue::glue("https://www.cbssports.com/nfl/schedule/2025/regular/{week_odds}"))%>%
    html_nodes(".TableBase-bodyTd:nth-child(1) a")%>%
    html_text() %>% 
...
read_html("https://www.sportsline.com/nfl/odds/money-line/")%>%
    # html_nodes("td:nth-child(8) .primary , .FqxOT .AA-Dxj")%>%
    html_nodes("td:nth-child(8) .secondary , .kLcZjE .dEoYge")%>%
    html_text() %>% 
    matrix(ncol = 2, byrow = TRUE) %>%
    as.data.frame() %>% 
    slice(1:games) %>% 
...
url <- "https://mab.chartbeat.com/mab_strategy/headline_testing/get_strategy/?host=sportsline.com&domain=sportsline.com&path=%2Fnfl%2Fodds%2F"  # replace with real one
res <- GET(url)
