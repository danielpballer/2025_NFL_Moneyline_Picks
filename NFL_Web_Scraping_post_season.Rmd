...
url = "https://www.cbssports.com/nfl/schedule/2025/regular/"  #base URL regular season
url_post = "https://www.cbssports.com/nfl/schedule/2025/postseason/"  #base URL post season
...
# Use unless timeouts.  Broken up below
# cbs_week_list %>%
...
#create a list of CBS urls
cbs_url_list = cbs_week_list%>%
  map(list_of_pages_cbs) 

#scrape the first half of the list
cbs_set1 = cbs_url_list[1:10] %>% 
  map2(cbs_week_list[1:10],.,cbs_week)%>%
  bind_rows() 

#scrape the second half of the list
cbs_set2 = cbs_url_list[11:current_week] %>% 
  map2(cbs_week_list[11:current_week],.,cbs_week)%>%
  bind_rows()

#Bind the 2 sets and save
bind_rows(cbs_set1, cbs_set2)%>% 
  write_csv(glue::glue("./CSV_Data_Files/CBS_Experts_{current_week}.csv"))
...
#scrape the first half of the list
cbs_season_set1 = cbs_url_list[1:10] %>% 
  map2(cbs_week_list[1:10],.,cbs_season)%>%
  bind_rows() 

#scrape the second half of the list
cbs_season_set2 = cbs_url_list[11:current_week] %>% 
  map2(cbs_week_list[11:current_week],.,cbs_season)%>%
  bind_rows() 

#Bind the 2 sets and save
bind_rows(cbs_season_set1, cbs_season_set2)%>% 
  write_csv(glue::glue("./CSV_Data_Files/CBS_Experts_Season_{current_week}.csv"))
...
last_week = reg_18 %>% slice(1:10) #to account that only 10 people are picking playoff games
